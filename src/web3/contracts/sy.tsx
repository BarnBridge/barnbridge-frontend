...
const CONTRACT_SY_ADDR = String(
  process.env.REACT_APP_CONTRACT_SY_ADDR,
).toLowerCase();
...
export type SYContract = SYContractData & {
  ...
  buyBond(principalAmount: BigNumber, minGain: BigNumber, deadline: number, forDays: number, gasPrice: number): Promise<void>;
};

const InitialData: SYContractData = {};

const Contract = new Web3Contract(
  require('web3/abi/sy.json'),
  CONTRACT_SY_ADDR,
  'SY',
);
...
function getBondGainCall(principalAmount: BigNumber, forDays: number): Promise<BigNumber> {
  return Contract
    .call('bondGain', [principalAmount, forDays])
    .then(value => {
...
function buyTokensSend(
  ...
  return Contract
    .send('buyTokens', [underlyingAmount, minTokens, deadline], {
      from,
...
function buyBondSend(
  ...
  return Contract
    .send('buyBond', [principalAmount, minGain, deadline, forDays], {
      from,
      gasPrice: getGasValue(gasPrice),
    });
}
...
export function useSYContract(): SYContract {
  ...
  const buyTokens = React.useCallback(
    (underlyingAmount: BigNumber, minTokens: BigNumber, deadline: number, gasPrice: number): Promise<void> => {
      ...
      return buyTokensSend(underlyingAmount, minTokens, deadline, wallet.account, gasPrice)
        .then(reload);
    },
    [reload, wallet.account],
  );
  ...
  const buyBond = React.useCallback(
    (principalAmount: BigNumber, minGain: BigNumber, deadline: number, forDays: number, gasPrice: number): Promise<void> => {
      if (!wallet.account) {
        return Promise.reject();
      }

      return buyBondSend(principalAmount, minGain, deadline, forDays, wallet.account, gasPrice)
        .then(reload);
    },
    [reload, wallet.account],
  );

  const getBondGain = React.useCallback(
    (principalAmount: BigNumber, forDays: number): Promise<BigNumber> => {
